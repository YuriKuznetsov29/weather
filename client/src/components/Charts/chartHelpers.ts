...
export function calkDayDuration(sunrise: string, sunset: string, hour: number, minutes: string) {
    ...
    const hours =
        sunsetMin - sunriseMin < 0 ? sunsetHour - sunriseHour - 1 : sunsetHour - sunriseHour
    const min =
        sunsetMin - sunriseMin < 0 ? 60 - Math.abs(sunsetMin - sunriseMin) : sunsetMin - sunriseMin
    const beforeSunsetHour = sunsetMin - +minutes < 0 ? sunsetHour - hour - 1 : sunsetHour - hour
    const beforeSunsetMin =
        sunsetMin - +minutes < 0 ? 60 - Math.abs(sunsetMin - +minutes) : sunsetMin - +minutes
